...
class ViewController: UIViewController {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let hitTest = self.sceneView.hitTest(view.center, types: .existingPlaneUsingExtent).first {
    ...
    func renderer(_ renderer: SCNSceneRenderer, updateAtTime time: TimeInterval) {
        DispatchQueue.main.async {
            if let _ = self.sceneView.hitTest(self.view.center, types: .existingPlaneUsingExtent).first {
                self.viewCross.backgroundColor = .green
            } else {
                self.viewCross.backgroundColor = .white
            }
        }
    }
